...
[フィヨルド](http://d.hatena.ne.jp/keyword/%A5%D5%A5%A3%A5%E8%A5%EB%A5%C9)ブートキャンプの課題で、こんなコードを書いていたら

> [ActiveRecord](http://d.hatena.ne.jp/keyword/ActiveRecord)はデフォルトでid同士を比較するので@user == current_userでOKです。

とレビューいただきました。

「[はえ](http://d.hatena.ne.jp/keyword/%A4%CF%A4%A8)〜そうなんや」ではもったいないので、ソースを追っかけてみました。
...
### [Ruby](http://d.hatena.ne.jp/keyword/Ruby)の[演算子](http://d.hatena.ne.jp/keyword/%B1%E9%BB%BB%BB%D2)の優先順位

[演算子](http://d.hatena.ne.jp/keyword/%B1%E9%BB%BB%BB%D2)ばっかりで面食らいました。
...
[Ruby](http://d.hatena.ne.jp/keyword/Ruby)は式全体の真偽を決定するまで、左辺から順に式を評価します。  
...
1つ目は、newしたばかりのレコードはidが[nil](http://d.hatena.ne.jp/keyword/nil)だから、  
2つ目は`select`でカラムを指定する際に`id`を対象外とすると、`id`は[nil](http://d.hatena.ne.jp/keyword/nil)だから、ということでしょうか。

> Note also that destroying a record preserves its ID in the model instance, so deleted models are still comparable.  
> また、レコードの削除はモデル[インスタンス](http://d.hatena.ne.jp/keyword/%A5%A4%A5%F3%A5%B9%A5%BF%A5%F3%A5%B9)のIDを保持するので、削除されたモデルはまだ比較可能であることにも注意してください。
...
- `id`が[nil](http://d.hatena.ne.jp/keyword/nil)の場合は、必ずfalseを返す
