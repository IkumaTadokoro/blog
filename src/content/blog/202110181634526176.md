...
[Ruby](http://d.hatena.ne.jp/keyword/Ruby)である文字列wordに数値が含まれているかどうかをBoolean値で判定するには次のようにする
...
`String#=~`は、[正規表現](http://d.hatena.ne.jp/keyword/%C0%B5%B5%AC%C9%BD%B8%BD)を引数にとり、マッチすればそのインデックスを、そうでなければ`nil`を返す。
