...
- [macOS](http://d.hatena.ne.jp/keyword/macOS):BigSur
- RubyMine:2020.3
- [Ruby](http://d.hatena.ne.jp/keyword/Ruby):2.7.1
- Rubocop:1.4.2
- parser:2.7.2.0
...
- Rubocopは[Ruby](http://d.hatena.ne.jp/keyword/Ruby)の[構文解析](http://d.hatena.ne.jp/keyword/%B9%BD%CA%B8%B2%F2%C0%CF)に[whitequark/parser: A Ruby parser.](https://github.com/whitequark/parser)を使用する
- [Ruby](http://d.hatena.ne.jp/keyword/Ruby)はパッチリリースごとに[構文解析](http://d.hatena.ne.jp/keyword/%B9%BD%CA%B8%B2%F2%C0%CF)へのバックポート[\*1](#f-51de6e0e)を盛り込むことがある。[構文解析](http://d.hatena.ne.jp/keyword/%B9%BD%CA%B8%B2%F2%C0%CF)を担うparserはこれに対応する必要がある。
- 上記に対応するため、parserは1つのバージョンで複数のマイナーバージョンをサポートできないそう（parser2.7.2.0では、Ruby2.7.2のみをサポートしていて、2.7.1をサポートしていない）
- そのため、[Ruby](http://d.hatena.ne.jp/keyword/Ruby)のバージョン（2.7.1）とparserの対応しているバージョン（2.7.2）が異なると警告が出る。
...
[Ruby](http://d.hatena.ne.jp/keyword/Ruby)に合わせてparserを2.7.2.0未満に下げることでエラーを解消します。
...
> [Ruby](http://d.hatena.ne.jp/keyword/Ruby) 2.7.2 has been released.[https://www.ruby-lang.org/en/news/2020/10/02/ruby-2-7-2-released/](https://www.ruby-lang.org/en/news/2020/10/02/ruby-2-7-2-released/)  
> It seems like no changes have been made to ruby27.y where Parser gem should be changed (backported).[ruby](http://d.hatena.ne.jp/keyword/ruby)/[ruby](http://d.hatena.ne.jp/keyword/ruby)@v2_7_1...v2_7_2
...
- `Ruby MRI`：CRuby（Matz' [Ruby](http://d.hatena.ne.jp/keyword/Ruby) Implementation）
...
- `parse.y`
  ...
  - `parse.y`は[Ruby](http://d.hatena.ne.jp/keyword/Ruby)の[構文解析](http://d.hatena.ne.jp/keyword/%B9%BD%CA%B8%B2%F2%C0%CF)のルールを記述したもの（記述されたPGは、字句解析→[構文解析](http://d.hatena.ne.jp/keyword/%B9%BD%CA%B8%B2%F2%C0%CF)の順に解析される）
  - [yacc](http://d.hatena.ne.jp/keyword/yacc)は、パーサを自動生成するツール。 `.y`という拡張子のファイルを入力とし、 Cのソースを出力する。
...
- [Ruby](http://d.hatena.ne.jp/keyword/Ruby)のバージョンアップごとにrubocopの[構文解析](http://d.hatena.ne.jp/keyword/%B9%BD%CA%B8%B2%F2%C0%CF)を担うparserもそれに追随するが、各バージョンのparserは複数のパッチバージョン (2.7 系, 2.6 系, 2.5 系, 2.4 系, 2.3 系...) のみ。
