...
<img alt="f:id:ikmbear:20200810155847p:plain" src="https://cdn-ak.f.st-hatena.com/images/fotolife/i/ikmbear/20200810/20200810155847.png" title="" height="630" width="1200" />
...
- [Ruby](http://d.hatena.ne.jp/keyword/Ruby) 2.7.0
...
これを[VSCode](http://d.hatena.ne.jp/keyword/VSCode)のターミナル（[zsh](http://d.hatena.ne.jp/keyword/zsh)）から実行すると...
...
<img alt="f:id:ikmbear:20200810154547p:plain" src="https://cdn-ak.f.st-hatena.com/images/fotolife/i/ikmbear/20200810/20200810154547.png" title="" height="66" width="806" />この実行結果は[VSCode](http://d.hatena.ne.jp/keyword/VSCode)のターミナル（[zsh](http://d.hatena.ne.jp/keyword/zsh)）で実行したものなので、環境起因の可能性がないかチェックします。

[mac](http://d.hatena.ne.jp/keyword/mac)標準のターミナル（[zsh](http://d.hatena.ne.jp/keyword/zsh)）で実行して結果を確認してみると、同じ現象が発生しています。

<img alt="f:id:ikmbear:20200810154611p:plain" src="https://cdn-ak.f.st-hatena.com/images/fotolife/i/ikmbear/20200810/20200810154611.png" title="" height="80" width="1054" />

一方、pryでの実行結果を確認すると、こちらは%は表示されていません。

<img alt="f:id:ikmbear:20200810154625p:plain" src="https://cdn-ak.f.st-hatena.com/images/fotolife/i/ikmbear/20200810/20200810154625.png" title="" height="82" width="760" />

つまり、「**%が表示されるのは[zsh](http://d.hatena.ne.jp/keyword/zsh)特有の現象**」であると言えそうです。

## [zsh](http://d.hatena.ne.jp/keyword/zsh)では改行のない結果行を出力する時に、プロンプト「%」を表示する

[zsh](http://d.hatena.ne.jp/keyword/zsh)特有だというヒントが得られたので、ドキュメントをチェックしてみます。
...
「PROMPT_CR オプションにより[コマンドプロンプト](http://d.hatena.ne.jp/keyword/%A5%B3%A5%DE%A5%F3%A5%C9%A5%D7%A5%ED%A5%F3%A5%D7%A5%C8)で覆われてしまうような部分行 (改行で終わらない行) を保存しようとする」

PROMPT_CRオプションについてはその上に記述があり、プロンプト（[zsh](http://d.hatena.ne.jp/keyword/zsh)では%）を表示する前にキャリッジリターン（CR）を印刷する（同一行の行頭に戻す）仕様のようです。
...
まず[ruby](http://d.hatena.ne.jp/keyword/ruby)プログラムを実行します。printメソッドは改行を出力しません。

```
% ./print_num.rb123456789
```

ここでPROMPT_CRオプションに基づいて、CRを印刷します。すると改行がされていないため、同一行の先頭に移動した結果、出力結果が消えます。これが「PROMPT_CR オプションにより[コマンドプロンプト](http://d.hatena.ne.jp/keyword/%A5%B3%A5%DE%A5%F3%A5%C9%A5%D7%A5%ED%A5%F3%A5%D7%A5%C8)で覆われてしまうような部分行」ということです。
...
PROMPT_SPオプションを無効にすることで、「PROMPT_CR オプションにより[コマンドプロンプト](http://d.hatena.ne.jp/keyword/%A5%B3%A5%DE%A5%F3%A5%C9%A5%D7%A5%ED%A5%F3%A5%D7%A5%C8)で覆われてしまうような部分行」を実際に確認することができます。
...
<img alt="f:id:ikmbear:20200810154709p:plain" src="https://cdn-ak.f.st-hatena.com/images/fotolife/i/ikmbear/20200810/20200810154709.png" title="" height="80" width="1078" />

部分行がCRにより覆われてしまい、見えなくなっていることがわかります。

## 「%」を排除するにはどうすればよいか

一つの手段は[Ruby](http://d.hatena.ne.jp/keyword/Ruby)側で改行を出力することです。最初のプログラムで
...
別の手段は、[zsh](http://d.hatena.ne.jp/keyword/zsh)の設定（\~/.zshrc）を書き換えてしまうことです。
...
- プログラム側で制御でき、環境に依存しないという点では[Ruby](http://d.hatena.ne.jp/keyword/Ruby)で改行を出力してしまうのがいいのかなあと思いました。
