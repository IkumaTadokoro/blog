...
import noUnusedVars from "../../assets/images/eslint-rules-customization/no-unused-vars.png"
import changeToWarn from "../../assets/images/eslint-rules-customization/change-to-warn.png"


## 課題： ESLintルールはそのままに、自分のエディタ上の表示を変更したい

とあるTypeScriptプロジェクトにおいて、「あるESLintをチェックすることはOKだが、視認性の関係でSeverityを変更したい」という要求がありました。

例えば`@typescript-eslint/no-unused-vars`を`error`に指定していると、使用していない変数がある場合にエディタ上で赤くハイライトされます。

<Image src={noUnusedVars} alt="未使用の変数hogeのVSCode上でホバーしている。Lintエラーであることを示す波線が変数名の下に出ており、エラーであるため赤い色で表示されている。" />

使われていない変数があることをチェックするのはルールとしてチェックしたいが、エラーではなく警告として表示したい、という要求です。
解決策の一つは愚直にルールのSeverityを変更することですが、これはプロジェクト全体に影響を与えるため、他の開発者にも影響を与えてしまいます。
...
もともとエラーの赤線だったものが、警告の黄色線に変わりました。
