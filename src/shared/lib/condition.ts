// refs: https://zenn.dev/shoalwave/articles/527a539d3c7a01

/**
 * union 型の差分を取得する（string, number, symbol のみ対応）
 */
type Diff<T extends keyof never, U extends T> = ({ [P in T]: P } & {
	[P in U]: never;
} & { [x: keyof never]: never })[T];
...
const matchedCondition = <T extends keyof never, R>(result: R): Condition<T, R> => {
	const conditionItem: ConditionItem<T, R> = {
		case: <U extends T, A>(_: U | Array<U>, __: (_: U) => A) =>
 ...
 */
const conditionImpl = <T extends keyof never, R>(val: T, result: R): Condition<T, R> => {
	const conditionItem: ConditionItem<T, R> = {
		case: <U extends T, A>(match: U | Array<U>, callback: (matched: U) => A) =>
			isUnMatch(val, match)
				? conditionImpl<Diff<T, U>, R | A>(val, result)
...
export const condition = <T extends keyof never>(val: T): Condition<T, never> =>
	conditionImpl(val, undefined as never);
